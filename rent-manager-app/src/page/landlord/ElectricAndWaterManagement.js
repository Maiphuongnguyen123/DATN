...
import { getAllElectricAndWaterOflandlord } from "../../services/fetch/ApiUtils";
...
const ElectricAndWaterManagement = (props) => {
  ...
  const fetchData = () => {
    getAllElectricAndWaterOflandlord(currentPage, itemsPerPage, searchQuery)
      .then((response) => {
  ...
  const handleEditElectric = (id) => {
    history(`/landlord/electric_water/edit/${id}`);
  };

  const handleRedirectAddElectric = () => {
    history(`/landlord/electric_water/add`);
  };

  const handleExportBill = (id) => {
    history(`/landlord/electric_water-management/export-bill/${id}`);
  };
