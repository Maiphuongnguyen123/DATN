...
function ContractManagement(props) {
  ...
  const fetchData = () => {
    getAllContractOflandlord(currentPage, itemsPerPage, searchQuery)
      .then((response) => {
  ...
  const handleRedirectAddRoom = () => {
    history("/landlord/add-contract");
  };

  const handleEditContract = (id) => {
    history("/landlord/edit-contract/" + id);
  };

  const handleExportBill = (id) => {
    history("/landlord/export-contract/" + id);
  };
